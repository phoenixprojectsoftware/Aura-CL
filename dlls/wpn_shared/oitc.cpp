...
void COne::Precache( void )
{
	...
	PRECACHE_SOUND("weapons/glock1.wav");
	PRECACHE_SOUND("weapons/glock2.wav");
	PRECACHE_SOUND("weapons/glock3.wav");
	PRECACHE_SOUND("weapons/glock4.wav");
	PRECACHE_SOUND("weapons/glock5.wav");
...
int COne::GetItemInfo(ItemInfo *p)
{
	...
	p->pszAmmo2 = NULL;
	p->iMaxAmmo2 = -1;
	p->iMaxClip = ONE_MAX_CLIP;
	p->iSlot = 1;
	p->iPosition = 0;
	p->iFlags = 0;
	p->iId = m_iId = WEAPON_ONE;
	p->iWeight = -15;
...
void COne::PrimaryAttack( void )
{
	GlockFire( 0.01, 0.3, TRUE );
}

void COne::GlockFire( float flSpread , float flCycleTime, BOOL fUseAutoAim )
{
	if (m_iClip <= 0)
	{
		if (m_fFireOnEmpty)
