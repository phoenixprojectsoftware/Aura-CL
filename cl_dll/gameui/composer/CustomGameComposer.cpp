...
{
	FileHandle_t f = g_pFullFileSystem->Open("composer_buffer.cfg", "wt", "mod");
	if (!f)
	{
		Warning("WriteComposerCfg() failed to open composer_buffer.cfg for writing\n");
		return false;
	}
...
void CCustomGameComposer::OnCommand(const char* command)
{
	if (!Q_stricmp(command, "startgame"))
	{
		...
		// building composer_buffer.cfg
		char out[4096];
		Q_snprintf(out, sizeof(out),
			"// Auto-generated by AURA Custom Game Composer\n"
			"sv_aura_gamemode %s\n"
			"mp_fraglimit %s\n"
			"mp_timelimit %s\n", gamemode, fraglimit, timelimit);

		if (!WriteComposerCfg(out))
		{
			Warning("CCustomGameComposer::OnCommand() failed to write composer_buffer.cfg\n");
			return;
		}
