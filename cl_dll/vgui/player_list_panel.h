...
#ifndef CPLAYERLISTPANEL_H
#define CPLAYERLISTPANEL_H
...
class CPlayerListSectionHeader;
class CPlayerListItemButton;

//-----------------------------------------------------------------------------
// Purpose: List panel control that is divided up into discrete sections
//-----------------------------------------------------------------------------
class CPlayerListPanel : public vgui2::Panel
{
	DECLARE_CLASS_SIMPLE(CPlayerListPanel, vgui2::Panel);

public:
	CPlayerListPanel(vgui2::Panel* parent, const char* name);
	~CPlayerListPanel();

	// sorting function, should return true if itemID1 should be displayed before itemID2
	typedef bool (*SectionSortFunc_t)(CPlayerListPanel* list, int itemID1, int itemID2);
	...
	*/
	...
	// image handling
	virtual void SetImageList(vgui2::ImageList* imageList, bool deleteImageListWhenDone);
...
protected:
	...
	virtual void OnMousePressed(enum vgui2::MouseCode code);
...
private:
	...
	void AddSectionHelper(int sectionID, CPlayerListSectionHeader* header, SectionSortFunc_t sortFunc);
	int GetSectionTall();
	...
	friend CPlayerListItemButton;
	void SetSelectedItem(CPlayerListItemButton* item);
	vgui2::DHANDLE<CPlayerListItemButton> m_hSelectedItem;
	...
	struct section_t
	{
		...
		CPlayerListSectionHeader* m_pHeader;
		CUtlVector<column_t> m_Columns;
		SectionSortFunc_t m_pSortFunc;
	};

	CUtlVector<section_t> m_Sections;
	CUtlLinkedList<CPlayerListItemButton*, int> m_Items;
	CUtlLinkedList<CPlayerListItemButton*, int> m_FreeItems;
	CUtlVector<CPlayerListItemButton*> m_SortedItems;

	vgui2::PHandle m_hEditModePanel;
