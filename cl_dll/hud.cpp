...
void CHud::StartDualSenseEffect(uint8_t triggerMask, uint8_t position, uint8_t amplitude, uint8_t frequency)
{
#ifdef _STEAMWORKS
	ISteamInput* steamInput = SteamInput();
	if (!steamInput)
	{
		gEngfuncs.Con_DPrintf("DualSense API not available.\n");
		return;
	}
	...
	gEngfuncs.Con_DPrintf("[DualSense] Setting trigger effect vibration:\n");
	gEngfuncs.Con_DPrintf("  Mask: 0x%02X | Position: %d | Amplitude: %d | Frequency: %d\n", triggerMask, position, amplitude, frequency);

	for (int i = 0; i < inputCount; ++i)
	{
		ScePadTriggerEffectParam param{};
		param.triggerMask = triggerMask;

		if (triggerMask & SCE_PAD_TRIGGER_EFFECT_TRIGGER_MASK_R2)
		{
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_R2].mode = SCE_PAD_TRIGGER_EFFECT_MODE_VIBRATION;
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_R2].commandData.vibrationParam.position = position;
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_R2].commandData.vibrationParam.amplitude = amplitude;
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_R2].commandData.vibrationParam.frequency = frequency;
			gEngfuncs.Con_DPrintf("  -> Applied to R2 for controller %llu\n", inputHandles[i]);
		}

		if (triggerMask & SCE_PAD_TRIGGER_EFFECT_TRIGGER_MASK_L2)
		{
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2].mode = SCE_PAD_TRIGGER_EFFECT_MODE_VIBRATION;
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2].commandData.vibrationParam.position = position;
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2].commandData.vibrationParam.amplitude = amplitude;
			param.command[SCE_PAD_TRIGGER_EFFECT_PARAM_INDEX_FOR_L2].commandData.vibrationParam.frequency = frequency;
			gEngfuncs.Con_DPrintf("  -> Applied to L2 for controller %llu\n", inputHandles[i]);
		}

		ESteamInputType type = steamInput->GetInputTypeForHandle(inputHandles[i]);
		gEngfuncs.Con_DPrintf("  Controller %llu type: %d\n", inputHandles[i], type);
		steamInput->SetDualSenseTriggerEffect(inputHandles[i], &param);
	}
#else
	gEngfuncs.Con_DPrintf("Steam API not present.\n");
#endif
}

void CHud::ClearDualSenseEffect()
{
#ifdef _STEAMWORKS
	ISteamInput* steamInput = SteamInput();
	if (!steamInput)
	...
	for (int i = 0; i < inputCount; ++i)
	{
		...
		param.command[0].mode = SCE_PAD_TRIGGER_EFFECT_MODE_NONE;
		param.command[1].mode = SCE_PAD_TRIGGER_EFFECT_MODE_NONE;
